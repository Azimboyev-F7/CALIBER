...
if (!username || !password || password.length < 6 || !email.includes('@')) throw new Error('Usage: node scripts/provisionAdmin.mjs <username> <password> [email]');
const url = process.env.VITE_SUPABASE_URL, serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
if (!url || !serviceKey) throw new Error('Supabase server credentials are not configured');
const supabase = createClient(url, serviceKey, { auth: { persistSession: false } });
const { data: users, error: listError } = await supabase.auth.admin.listUsers({ perPage: 1000 });
if (listError) throw listError;
let user = users.users.find((candidate) => candidate.email?.toLowerCase() === email || candidate.user_metadata?.username?.toLowerCase() === username);
if (user) {
