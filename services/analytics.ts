...
export async function recordUsage(userId: string, event: UsageEvent): Promise<boolean> {
  try {
...
export function trackSuccessfulAction(req: Request, res: Response, next: NextFunction) {
  ...
  if (event && res.locals.userId) {
    ...
    res.once('finish', () => {
      if (res.statusCode >= 200 && res.statusCode < 300) void recordUsage(userId, event);
    });
  }
  next();
}
